---
title : "Journey"
menu: "main"
date: "2025-12-27"
showTableOfContents: true
---

## 💼 Professional Background

{{< alert >}}
**TODO** next, add total years working in the companies to ease the readers
{{< /alert >}}


{{< timeline src="work_career" >}}

---

## 🎓 Educational Background

{{< alert >}}
**TODO** Fix the splitting lines to enhance readibility
{{< /alert >}}



{{< timeline src="edu_career" >}}

---

## 📢 Language Proficiency

{{< info_card
  title="IELTS"
  text="Score  6.5 (CERF B2)<br> Issued by IDP IELTS <br> Issue Date 5 Dec 2025"
  href="https://drive.google.com/file/d/1Gfn7t9jXF9w9COKOZiw_GC-WEDmD4Z0d/view?usp=sharing"
  cta="Preview Certificate"
>}}


---

## ⚙️ Certification

{{< alert >}}
**TODO** don't forget to update the certification
{{< /alert >}}


| Course              | Date     | Issued by |
|---------------------|----------|-----------|
| Python Introduction | 2 Dec'25 | LinkedIn  |
| Python Introduction | 2 Dec'25 | Datacamp  |
| Python Introduction | 2 Dec'25 | Sololearn |

## Resume

{{< button href="https://drive.google.com/file/d/1THHRr1Q9f30gr6BeknUTKtjSN7JmMh1H/view?usp=sharing" >}}
Resume
{{< /button >}}
